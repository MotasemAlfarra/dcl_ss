...
BASE_PATH = '/export/share/datasets/BDD100K/seg/images'
class BDD100K(data.Dataset):
    ...
    def __init__(self, root, train=True, transform=None, domain_transform=None):
        ...
        self.images = [  # Add train cities
            (
                path,
                path.replace('images', 'labels').replace('.jpg', '_train_id.png'),
                -1
            ) for path in sorted(glob.glob(os.path.join(train_folder, "*.jpg")))
        ]
        self.images += [  # Add val cities
            (
                ...
                path.replace('images', 'labels').replace('.jpg', '_train_id.png'),
                -1
            ) for path in sorted(glob.glob(os.path.join(val_folder, "*.jpg")))
        ]
...
class BDD100K_Incremental(data.Dataset):
