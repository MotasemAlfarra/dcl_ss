...
from .map_idd_to_cs import transform_IDD_Segmentation_mask_to_CityScapes_Segmentation_mask as transform_mask
from .utils import Subset

BASE_PATH = '/export/share/datasets/IndianDrivingDataset/IDD_Segmentation/leftImg8bit'
class IDD(data.Dataset):
    ...
    def __init__(self, root='./', train=True, transform=None, domain_transform=None):
        ...
        self.images = [  # Add train cities
            (
                path,
                path.replace('leftImg8bit', 'gtFine').replace('.png', '_labelids.png'),
                -1
            ) for path in sorted(glob.glob(os.path.join(train_folder, "*/*.png")))
        ]
        self.images += [  # Add val cities
            (
                ...
                path.replace('leftImg8bit', 'gtFine').replace('.png', '_labelids.png'),
                -1
            ) for path in sorted(glob.glob(os.path.join(val_folder, "*/*.png")))
        ]
...
class IDD_Incremental(data.Dataset):
