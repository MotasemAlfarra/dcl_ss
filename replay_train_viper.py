...
class Trainer:
    ...
    def train(self, cur_epoch, optim, train_loader, scheduler=None, print_int=10, logger=None, reg=None, save_path=None, replay_buffer=None, synth_loader=None):
        ...
        random_number = 0
        model.train()
        synth_loader_iter = iter(synth_loader)
        print('self.numeber of internal steps is:', self.num_internal_steps)
        for cur_step, (images, labels) in enumerate(train_loader):
            
            #Doing stuff for the replay buffer
            # print("Images and labels shapes are {}, {}".format(images.shape, labels.shape))
            synth_images, synth_labels = synth_loader_iter.next()
            images = torch.cat([images, synth_images], dim=0)
            labels = torch.cat([labels, synth_labels], dim=0)
            if replay_buffer is not None:
                if cur_step > 0:# You cannot sample from an empty buffer
                    buffer_images, buffer_labels = replay_buffer.sample()
                replay_buffer.update(images, labels)
                if cur_step > 0:# Update images and labels after updating buffer
